...
class Document:
    ...
    def to_serialised_string(self):
        ...
        """
        return marshal.dumps((self.text,[sentence.to_dict() for sentence in self.sentences]))

    def from_serialised_string(serialised_string:str):
        """ Create and initialise a new document from a serialised_string generated by Document.to_serialised_string():
        """
        try:
            string = marshal.loads(serialised_string)
            ret = Document(string[1])
            ret.text = string[0]
            ret.build_ents()
            return ret
        except:
            raise Exception(f"Could not crete new Document fron serialised string")
...
class Token:
    ...
    def to_dict(self, fields=[ID, TEXT, NER, MISC]):
        ...
        for word in self.words:
            ...
            token_dict[NER] = getattr(self, NER)
            ret.append(token_dict)
        return ret
